...
from ..modules import BasicConv2d, BasicConv3d, TemporalBlock, HorizontalPoolingPyramid, SeparateFCs, SeparateBNNecks, SetBlockWrapper, PackSequenceWrapper, SelfSupervisedAlignmentModule, MaskedPartReconstructionModule


class SpatialAlignTemporalOcclusionRecover(BaseModel):
    def build_network(self, model_cfg):
        in_c = model_cfg['in_channels']
        time_dim = model_cfg['time_dim']
        self.in_c = in_c
        self.part_num = sum(model_cfg['bin_num'])
        ...
        self.MaskedReconModule = PackSequenceWrapper(MaskedPartReconstructionModule(**model_cfg['MaskedReconModule']))
        self.nonOccNorm = nn.BatchNorm1d(in_c[3], affine=False)
        self.occNorm = nn.BatchNorm1d(in_c[3], affine=False)
    ...
    def forward(self, inputs):
        ...
        outs_part, outs_part_recon, outs_part_recover = self.MaskedReconModule(outs, seqL) # [s, 1, c, p]

        if self.training:
            nonOcc_mask = (occ_labs.view(-1, 1, 1) == 0)

            occ_mask = ~nonOcc_mask
            outs_part_occ = torch.masked_select(outs_part.view(-1, c, self.part_num), occ_mask).contiguous().view(-1, c, self.part_num)
            outs_part_recon_occ = torch.masked_select(outs_part_recon.view(-1, c, self.part_num), occ_mask).contiguous().view(-1, c, self.part_num)
            outs_part_occ = self.occNorm(outs_part_occ)
            outs_part_recon_occ = self.occNorm(outs_part_recon_occ)

            outs_part_nonOcc = torch.masked_select(outs_part.view(-1, c, self.part_num), nonOcc_mask).contiguous().view(-1, c, self.part_num)
            outs_part_recon_nonOcc = torch.masked_select(outs_part_recon.view(-1, c, self.part_num), nonOcc_mask).contiguous().view(-1, c, self.part_num)
            outs_part_nonOcc = self.nonOccNorm(outs_part_nonOcc)
            outs_part_recon_nonOcc = self.nonOccNorm(outs_part_recon_nonOcc)

        # Temporal Pooling, TP
        rec_outs = self.TP(outs_part_recover.permute(1, 0, 2, 3).contiguous(), seqL, dim=1)[0]  # [n, c, p]
        outs = self.TP(outs, seqL, dim=1)[0]  # [n, c, h, w]

        # Horizontal Pooling Matching, HPM
        feat = self.HPP(outs) + rec_outs # [n, c, p]
        ...
        if self.training:
            n, s, _, h, w = sils.size()
            # print("sils:{}".format(sils.size()))
            retval = {
                'training_feat': {
                    'triplet': {'embeddings': embed_1, 'labels': labs},
                    'softmax': {'logits': logits, 'labels': labs},
                    'affine': {'theta': theta, 'theta_gt': theta_gt, 'x': sils_selfSuperTrans, 'x_gt': sils_gt},
                    'nonOccRecMSE': {'nonOcc_x': outs_part_nonOcc, 'nonOcc_y': outs_part_recon_nonOcc, 'occ_x': outs_part_occ, 'occ_y': outs_part_recon_occ}
                },
                'visual_summary': {
